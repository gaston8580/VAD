...
class MapDetectionTransformerDecoderTRT(TransformerLayerSequence):
    ...
    def __init__(self, *args, return_intermediate=False, **kwargs):
        super(MapDetectionTransformerDecoderTRT, self).__init__(*args, **kwargs)
...
class VADPerceptionTransformerTRT(BaseModule):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(VADPerceptionTransformerTRT, self).__init__(**kwargs)
    ...
    def get_bev_features(
        ...
        for lvl, feat in enumerate(mlvl_feats):
            ...
            if self.use_cams_embeds:
                feat = feat + self.cams_embeds[:, None, None, :].to(feat.dtype) # TODO: cams_embeds和环视相机的径向对称性有没有关系？20240613
            feat = feat + self.level_embeds[None, None, lvl:lvl + 1, :].to(feat.dtype)
            spatial_shapes.append(spatial_shape)
            feat_flatten.append(feat)

        # 对每个 camera 的所有层级特征进行聚合
    ...
    def forward(self,
        ...
        Args:
            mlvl_feats (list(Tensor)): Input queries from different level. Each element has shape
                [bs, num_cams, embed_dims, h, w].
            bev_queries (Tensor): (bev_h*bev_w, c)
            bev_pos (Tensor): (bs, embed_dims, bev_h, bev_w)
            object_query_embed (Tensor): The query embedding for decoder, with shape [num_query, c].
            reg_branches (obj:`nn.ModuleList`): Regression heads for feature maps from each decoder layer. Only would
                be passed when `with_box_refine` is True. Default to None.
        Returns:
            tuple[Tensor]: results of decoder containing the following tensor.
                - bev_embed: BEV features
                - inter_states: Outputs from decoder. If return_intermediate_dec is True output has shape
                      (num_dec_layers, bs, num_query, embed_dims), else has shape (1, bs, num_query, embed_dims).
                - init_reference_out: The initial value of reference points, has shape (bs, num_queries, 4).
                - inter_references_out: The internal value of reference points in decoder, has shape
                    (num_dec_layers, bs,num_query, embed_dims)
                - enc_outputs_class: The classification score of proposals generated from encoder's feature maps, 
                    has shape (batch, h*w, num_classes). Only would be returned when `as_two_stage` is True, 
                    otherwise None.
                - enc_outputs_coord_unact: The regression results generated from encoder's feature maps., has shape
                    (batch, h*w, 4). Only would be returned when `as_two_stage` is True, otherwise None.
        """
        ...
        query = query.permute(1, 0, 2)  # [num_agent, bs, embed_dims], agent_query
        query_pos = query_pos.permute(1, 0, 2)  # [num_agent, bs, embed_dims]
        map_query = map_query.permute(1, 0, 2)  # [num_pl, bs, embed_dims]
        map_query_pos = map_query_pos.permute(1, 0, 2)  # [num_pl, bs, embed_dims]
...
@TRANSFORMER_LAYER_SEQUENCE.register_module()
class CustomTransformerDecoderTRT(TransformerLayerSequence):
    """Implements the decoder in DETR3D transformer.
    Args:
        return_intermediate (bool): Whether to return intermediate outputs.
    ...
    def __init__(self, *args, return_intermediate=False, **kwargs):
        super(CustomTransformerDecoderTRT, self).__init__(*args, **kwargs)
        self.return_intermediate = return_intermediate
        self.fp16_enabled = False
